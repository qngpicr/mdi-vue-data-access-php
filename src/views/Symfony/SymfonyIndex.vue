<template>
  <div class="container mt-4">
    <h2 style="margin-top:20px">RouteInfoController (Symfony)</h2>
    ...
    <table class="table table-bordered table-sm table-hover">
      ...
      <tbody>
        <tr v-for="r in apiRoutes" :key="r.uri + r.method">
          <td>{{ r.method }}</td>
          <td>
            <a :href="`https://localhost:9014/${r.uri}`" target="_blank">
              /symfony/{{ r.uri }}
            </a>
          </td>
        </tr>
...
const { otherRoutes, apiRoutes, csrRoutes } = useModuleIndex(9014) // Symfony
</script>
